    ...
    <section class="relative">
        ...
        <div class="relative text-white py-16 md:py-20 bg-gradient-to-r from-blue-600 via-teal-500 to-emerald-500 shadow-lg">
            <div class="max-w-6xl mx-auto px-4 text-center">
                <h1 class="text-3xl md:text-5xl font-extrabold tracking-wide">{{ $umkm->farm }}</h1>
            </div>
        </div>
        ...
        <div class="container mx-auto py-10 px-4">
            <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                ...
                <div class="p-8 space-y-6">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800">{{ $umkm->farm }}</h1>
                        <p class="mt-2 text-gray-500">Informasi lengkap mengenai UMKM ini</p>
                    </div>

                    {{-- Info Peternakan --}}
                    <div class="grid md:grid-cols-2 gap-4 text-gray-700">
                        <p><strong>Jenis Umkm:</strong> {{ $umkm->jenis_umkm }}</p>
                        <p><strong>Jumlah Umkm:</strong> {{ $umkm->jumlah_umkm }}</p>
                        <p><strong>Pemilik:</strong> {{ $umkm->pemilik }}</p>
                        <p><strong>Lokasi:</strong> {{ $umkm->alamat }}</p>
